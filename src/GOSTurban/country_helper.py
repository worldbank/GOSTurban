...
class urban_country():
    ...
    def __init__(self, iso3, sel_country, cur_folder, inP):
        ...
        self.urban_extents_file        = os.path.join(cur_folder, f"{iso3}_urban_extents.geojson")
        self.urban_extents_raster_file = os.path.join(cur_folder, f"{iso3}_urban_extents.tif")
        self.urban_extents_hd_file     = os.path.join(cur_folder, f"{iso3}_urban_extents_hd.geojson")
        self.urban_extents_hd_raster_file = os.path.join(cur_folder, f"{iso3}_urban_extents_hd.tif")
    ...
    def calculate_urban_extents(self, calculate_area=True, area_crs=3857):
        ...
        if not os.path.exists(self.urban_extents_file):
            ...
            urban_extents = urban_calculator.calculateUrban(densVal=300, totalPopThresh=5000, 
                                                       ...
                                                       verbose=True, raster=self.urban_extents_raster_file)
            # Calculate area of urban extents
            if calculate_area:
                urban_extents = urban_extents.to_crs(area_crs)
                urban_extents = urban_extents.to_crs(area_crs)
                urban_extents['area_km'] = urban_extents['geometry'].apply(lambda x: x.area/1000000)
        ...
        if not os.path.exists(self.urban_extents_hd_file):    
            urban_extents_hd = urban_calculator.calculateUrban(densVal=1500, totalPopThresh=50000, 
                                                       ...
                                                       verbose=True, raster=self.urban_extents_hd_raster_file)
            # Calculate area of urban extents
            if calculate_area:
                urban_extents_hd = urban_extents_hd.to_crs(area_crs)
                urban_extents_hd = urban_extents_hd.to_crs(area_crs)
                urban_extents_hd['area_km'] = urban_extents_hd['geometry'].apply(lambda x: x.area/1000000)
    ...
    def summarize_ntl(self, ntl_files = []):
        ...
        '''
        if (not os.path.exists(self.urban_ntl)) or (not os.path.exists(self.urban_hd_ntl)):
            if len(ntl_files) == 0:
    ...
    def summarize_ghsl(self, ghsl_files, binary_calc=False, binary_thresh=1000, clip_raster=False):
        ''' Summarize GHSL data
        ...
        '''
        if (not os.path.exists(self.urban_ghsl)) or (not os.path.exists(self.urban_hd_ghsl)):
            try:
                urbanD = self.urban_extents
                urbanHD = self.urban_extents_hd
            except:
            ...
            for ghsl_file in ghsl_files:
                ...
                res_urban = pd.DataFrame(res_urban, columns=["SUM","MIN","MAX","MEAN"])
                urbanD[f'ghsl_{date}'] = res_urban['SUM']                 
                
                res_hd_urban = rMisc.zonalStats(urbanHD, inR, minVal=0)
                res_hd_urban = pd.DataFrame(res_hd_urban, columns=["SUM","MIN","MAX","MEAN"])
                ...
                if binary_calc: # run zonal stats on a binary built layer
                    try:
                        ...
                        raise(ValueError("In order to calculate binary zonal, you need to clip out local ghsl data"))
    ...
    def delete_urban_data(self):
        ...
        '''                
        for cFile in [self.urban_extents_file, self.urban_extents_raster_file, 
