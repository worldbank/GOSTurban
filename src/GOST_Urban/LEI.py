...
def mp_lei(curRxx, transformxx, idx_xx, old_list=[4,5,6], new_list=[3], buffer_dist=300):
    ''' calculate and summarize LEI for curRxx, designed for use in multiprocessing function '''
    curRes = calculate_LEI(curRxx, transform=transformxx, old_list=old_list, new_list=new_list, buffer_dist=buffer_dist)
...
def lei_from_feature(inD, inR, old_list = [4,5,6], new_list=[3], buffer_dist=300, transform='', nCores=0, measure_crs=None, idx_col=None):
    ...
    for idx, row in inD.iterrows():
        ...
        if (np.isin(curR, old_list).sum() > 2) & (np.isin(curR, new_list).sum() > 2):
            if measure_crs is None:
                transform = rasterio.transform.from_bounds(*row['geometry'].bounds, curR.shape[0], curR.shape[1])
            else:
                transform = rasterio.transform.from_bounds(*measureD.loc[idx,'geometry'].bounds, curR.shape[0], curR.shape[1])        
            cur_idx = idx
            if idx_col:
                cur_idx = row[idx_col]
...
def calculate_LEI(inputGHSL, old_list, new_list, buffer_dist=300, transform=''):
    ...
    allVals = []
    for geom, value in rasterio.features.shapes(newR.astype('uint8'), transform=transform):
        if value == 1:
